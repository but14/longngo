...
void add_employee(struct nhanvien* list, int* count) {
    ...
    for (int i = 0; i < *count; i++) {
        if (list[i].manv == new_employee.manv) {
            printf("Error: Da co ma nhan vien này, vui long nhap lai.\n");
            duplicate_code = 1;
            break;
        }
    }
    ...
    if (!duplicate_code) {
        ...
        printf("CHUC VU: ");
        scanf("%s", new_employee.chucvu);
        
        printf("SO PHONG: ");
        scanf("%s", new_employee.phong);
        list[*count] = new_employee;
        (*count)++;
        printf("THEM THANH CONG NHAN VIEN MOI VAO DANH SACH.\n");
    }
}
...
void ghifile(struct nhanvien* ds, int  slnv){
	FILE* fOut=fopen("dsnhanvien.txt","a" );
	int i;
	for(i = 0;i<slnv;i++){
